        ...
        <template v-slot:top>
          <v-toolbar flat>
            <v-toolbar-title style="color: #01579b"
              >Proyectos investigación</v-toolbar-title
            >
                  ...
                  <v-container>
                    ...
                    >
                      <v-row>
                        ...
                        <v-col cols="12" sm="6" md="4" v-else>
                          <v-card-title style="font-size: 16px"
                            >Clasificación</v-card-title
                          >

                          <v-card-subtitle>
                            {{ editedItem.clasificacion }}
                          </v-card-subtitle>
                        </v-col>
                      </v-row>
                    </v-form>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Tareas Proyecto investigacions', value: 'clasificacion' },
  ...
  methods: {
    async initialize() {
      try {
        const investigacionUrl = await this.$axios.$get('/api/investigacion')
        this.investigacion = investigacionUrl.map((item) => {
          ...
          return {
            ...
            tareas_impacto_url: `${this.$axios.defaults.baseURL}/download/investigacion/${item.idProyecto}/tareas_impacto/`,
          }
        })
      } catch (error) {
        console.error(error)
    ...
    async downloadFile(item) {
      try {
        const response = await fetch(item.tareas_impacto_url)
        if (!response.ok) {
          throw new Error(
            `El archivo "${item.tareas_impacto}" no se pudo encontrar`
          )
        }
        ...
        aux2.href = url
        aux2.download = item.tareas_impacto
    ...
    editItem(item) {
      this.editedIndex = this.investigacion.indexOf(item)
      ...
      this.editItem.tareas_impacto = null
      this.dialog = true
    },
    showItem(item) {
      this.editedIndex = this.investigacion.indexOf(item)
    ...
    async deleteItemConfirm() {
      const idProyecto = this.investigacion[this.editedIndex].idProyecto

      try {
        await this.$axios.$delete(`/api/investigacion/${idProyecto}`)
        await this.initialize()
        this.closeDelete()
      } catch (error) {
        console.error(error)
    ...
    async save() {
      ...
      const { editedItem, editedIndex, investigacion } = this
      const url = '/api/investigacion/'
      const id =
        editedIndex >= 0 ? investigacion[editedIndex].idProyecto + '/' : ''
      ...
      formData.append('clasificacion', editedItem.clasificacion)
      try {
        if (editedIndex > -1) {

      ...
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title style="color: #01579b"
            >Profesores por asignatura</v-toolbar-title
          >
                    ...
                    <v-row style="padding: 0px">
                      ...
                      <v-col cols="12" sm="6" md="4">
                        <v-select
                          ...
                          :item-text="(item) => `${item.nombre_trabajador} ${item.apellido}`"
                        ></v-select>
                      </v-col>
...
export default {
  ...
  async created() {
    await Promise.all([this.getProfesor(), this.getAsignatura()])
  ...
  methods: {
    async initialize() {
      try {
        const asignatura_profesorUrl = await this.$axios.$get(
          '/api/asignatura_profesor'
        )

        this.asignatura_profesor = asignatura_profesorUrl.map((item) => {
          ...
          const profesorAux = prof
            ...
            : null
          return {
            idAsignatura_profesor: item.idAsignatura_profesor,
    ...
    editItem(item) {
      this.editedIndex = this.asignatura_profesor.indexOf(item)
    ...
    async deleteItemConfirm() {
      const idAsignatura_profesor =
        this.asignatura_profesor[this.editedIndex]
          .idAsignatura_profesor

      try {
        await this.$axios.$delete(
          `/api/asignatura_profesor/${idAsignatura_profesor}`
        )
        await this.initialize()
        this.closeDelete()
      } catch (error) {
   ...
   async save() {
      ...
      let { editedItem, editedIndex, asignatura_profesor } = this
      const url = '/api/asignatura_profesor/'
      const id =
        editedIndex >= 0
          ? asignatura_profesor[editedIndex].idAsignatura_profesor
          : ''
      const requestData = {

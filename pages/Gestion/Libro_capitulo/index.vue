<template>
  <v-container mt-5>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title style="color: #01579b">Libros o capítulos</v-toolbar-title>
                ...
                <v-container>
                  <v-form ref="form" lazy-validation>
                    <v-row>
                      ...
                      <v-col cols="12" sm="6" md="4">
                        <v-text-field
                          v-model="editedItem.lugar_publicacion"
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: 'Lugar publicación', value: 'lugar_publicacion' },
  ...
  methods: {
    async initialize() {
      try {
        const libroUrl = await this.$axios.$get('/api/libro')
        this.libro = libroUrl.map((item) => {
          return {
            ...
            lugar_publicacion: item.lugar_publicacion,
    ...
    editItem(item) {
      this.editedIndex = this.libro.indexOf(item)
    ...
    async deleteItemConfirm() {
      const idPublicacion = this.libro[this.editedIndex].idPublicacion

      try {
        await this.$axios.$delete(`/api/libro/${idPublicacion}`)
    ...
    async save() {
      ...
      let { editedItem, editedIndex, libro } = this
      const url = '/api/libro/'
      const id =
        editedIndex >= 0 ? libro[editedIndex].idPublicacion : ''
      const requestData = {
        titulo: editedItem.titulo,
        fecha_publicacion: editedItem.fecha_publicacion,
        autor: editedItem.autor,
        edicion: editedItem.edicion,
        lugar_publicacion: editedItem.lugar_publicacion,
        editorial: editedItem.editorial,
      }
      try {
        if (editedIndex > -1) {
